...
class ShellWrapperUnitTests(TestCase):
    ...
    def test_posix_basic(self):
        ...
        with env_vars(
            ...
            new_path = activator.pathsep_join(
                activator._remove_prefix_from_path(self.prefix)
            )
            (
                conda_exe_export,
                conda_exe_unset,
    ...
    #                 "activate1": activator.path_conversion(
    #                     join(
    #                         self.prefix, "etc", "conda", "activate.d", "activate1.fish"
    #                     )
    #                 ),
    #                 "deactivate1": activator.path_conversion(
    #                     join(
    ...
    #             "activate1": join(
    #                 self.prefix, "etc", "conda", "activate.d", "activate1.ps1"
    #             ),
    #             "conda_exe_export": conda_exe_export,
    #         }
    #     )
...
class ShellWrapperIntegrationTests(TestCase):
    ...
    def setUp(self):
        ...
        if not which("git") or which("git").startswith(sys.prefix):
            log.warning(
                "Installing `git` into {} because during these tests"
                ...
                "the original sys.prefix conda env falls off of it.".format(sys.prefix)
            )
    ...
    def basic_posix(self, shell):
        ...
        assert (
            _CE_M2 == _CE_M2
        ), "_CE_M changed by activation procedure\n:From\n{}\nto:\n{}".format(
            _CE_M, _CE_M2
        )
        assert (
